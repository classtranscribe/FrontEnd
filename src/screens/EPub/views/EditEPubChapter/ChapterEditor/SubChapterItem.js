...
function SubChapterItem({ subChapter, subChapterIndex, currChIndex, dispatch }) {
  ...
  const onSaveTitle = (newTitle) =>
    dispatch({
      ...
      payload: {
        action: 'saveSubChapterTitle',
        payload: { chapterIdx: currChIndex, subChapterIdx: subChapterIndex, value: newTitle },
      },
    });
  ...
  const onRemove = (index) => () => {
    dispatch({
      ...
      payload: {
        ...
        payload: { contentIdx: index, subChapterIdx: subChapterIndex },
      },
    });
  };
  ...
  const onTextChange = (index) => (val) => {
    dispatch({
      ...
      payload: {
        ...
        payload: { contentIdx: index, subChapterIdx: subChapterIndex, value: val },
      },
    });
  };
  ...
  const onImageChange = (index) => (val) => {
    dispatch({
      ...
      payload: {
        ...
        payload: { contentIdx: index, subChapterIdx: subChapterIndex, value: val, type: 'image' },
      },
    });
  };
  ...
  return (
    <CTFragment id={epub.id.schID(id)} className="mt-5">
      <CTText muted className="pl-1">
        Sub-Chapter {subChapterIndex + 1}: {title}
      </CTText>
      <ChapterTitle
