...
function NavMenuItem({ isSubCh, id, chapter, chIdx, schIdx, navId, dispatch }) {
  ...
  return (
    <li aria-current={current ? 'true' : 'false'}>
      <Link title={navTxt} id={navItemId} to={navLink} className={liClasses} onClick={onNavigate}>
...
function NavigationMenu({ view, navId, currChIndex, chapters = [], dispatch }) {
  ...
  return (
    <ul className="plain-ul" id={ID.EPubNavigationMenuID}>
      {chapters.map((ch, chIdx) => (
        <Fragment key={ch.id}>
          <NavMenuItem id={ch.id} chIdx={chIdx} chapter={ch} navId={navId} dispatch={dispatch} />
          {ch.subChapters.length > 0 &&
            (!mightHideSubCh || (mightHideSubCh && chIdx === currChIndex)) && (
