...
function insertChapter(chapters, index, chapterLike) {
  ...
  return [...chapters.slice(0, index), newChapter, ...chapters.slice(index)];
}
function removeChapter(chapters, index) {
  console.log(`Removing chapter at ${index}`);
  // let chapter = chapters[index];
  return [...chapters.slice(0, index), ...chapters.slice(index + 1)];
}
...
export default {
  ...
  appendChapterAsSubChapter(state, { payload: { chapterIdx } }) {
    ...
    return {
      ...
      epub: {
        ...state.epub,
        ...nextStateOfChapters(appendChapterAsSubChapter(chapters, chapterIdx)),
      },
    };
  },
  ...
  splitChaptersByScreenshots(state) {
    ...
    let splitChapters = _.map(state.items, (data, idx) =>
      new EPubChapterData({
        items: [data],
        title: data.title,
      }).toObject(),
  ...
  resetToDefaultChapters(state) {
    ...
    return {
      ...state,
      epub: { ...state.epub, ...nextStateOfChapters(defaultChapters) },
      currChIndex: 0,
    };
  },
  insertChapterContent(state, { payload: { type = 'text', contentIdx, subChapterIdx, value } }) {
    ...
    if (subChapterIdx === undefined) {
      ...
      console.log(
        `Inserting chapter ${state.currChIndex} subchapter ${subChapterIdx} content: `,
        value,
      );
      insertContentChapter(
        chapters?.[state.currChIndex]?.subChapters?.[subChapterIdx],
        contentIdx,
        value,
      );
    }
  ...
  setChapterContent(state, { payload: { type = 'text', contentIdx, subChapterIdx, value } }) {
    ...
    if (subChapterIdx === undefined) {
      ...
      console.log(
        `Setting chapter ${state.currChIndex} subchapter ${subChapterIdx} content: `,
        value,
      );
    }
