...
export class InstructorOffering extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.playlists !== this.state.playlists || prevState.courseOffering !== this.state.courseOffering) {
      ...
      const isExactOfferingPage = !window.location.pathname.includes('playlist') && !window.location.pathname.includes('data')
  ...
  render() {
    ...
    return (      
      <main className="op-bg"> 
        ...
        <Route path={`/offering/${this.id}/video-rename/:mediaId`} component={VideoEditing} />
                ...
                <TransitionGroup>
                  <CSSTransition key={location.key} classNames="fade" timeout={300}>
                    <Switch location={location}>
                      ...
                      {/* Analytics page */}
                      <Route exact path={`/offering/${this.id}/data`} render={props => 
                        <Analytics {...props} {...this.state} offeringId={this.id} />
                      }/>
