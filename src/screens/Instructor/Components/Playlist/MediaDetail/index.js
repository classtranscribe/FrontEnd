...
function MediaDetail({ media, onClose }) {
  ...
  return (
    <div id="ip-p-md" ref={thisRef}>
      ...
      {/* Media Name */}
      <div id="ip-p-md-info">
        <button className="plain-btn ip-goback" onClick={handleClose}>
          <span tabIndex="-1">
        ...
        {createdAt && (
          <div className="text-muted mb-1 mt-3">Created at {createdAt.slice(0, 10)}</div>
        )}
        <h3
          ref={nameRef}
        ...
        <div className="ct-d-r-end">
          {isEditing ? (
            <Button uppercase text="save" color="transparent teal" onClick={handleRename} />
          ) : (
            <>
              <Button.Group>
                <Button
