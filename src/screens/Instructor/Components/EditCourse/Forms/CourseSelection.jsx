...
function CourseSelectionWithRedux({
  ...
  useEffect(() => {
    if (depart) {
      api.getCoursesByDepartId(depart).then(({ data }) => {
        // console.log('courses', data, depart)
        let dep = _.find(departments, { id: depart });
        setCourses(_.map(data, (co) => ({ ...co, acronym: dep.acronym })));
      });
    }
  }, [depart]);
    ...
    <div className="ip-f-section">
      ...
      <Grid columns="equal" stackable className="ip-f-grid">
        <Grid.Row>
          ...
          {Boolean(courses.length > 0 || selCourses.length > 0) && <Grid.Column />}
        </Grid.Row>
          ...
          <Grid.Row className="ct-a-fade-in">
            ...
            <Grid.Column>
              <div className="w-100 ip-f-seled-course-con">
                ...
                {selCourses.length === 0 && (
                  <div className="ip-f-dtext mt-4">Your selected courses will show here</div>
                )}
...
export const CourseSelection = connectWithRedux(CourseSelectionWithRedux, [
  'departments',
  'offering',
]);
