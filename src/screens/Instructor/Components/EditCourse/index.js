...
import { setup, offControl, connectWithRedux, NEW_OFFERING } from '../../Utils';

import { CourseSelection, BasicInfo, Students, Staffs } from './Forms';
import './index.css';
      ...
      <div className="ip-edit-c-con">
        ...
        <div className="ip-f-form-con">
          <form className="w-100">
            <CourseSelection errors={errors} setErrors={setErrors} />

            <BasicInfo errors={errors} setErrors={setErrors} setAddStudents={setAddStudents} />

            {addStudents && <Students errors={errors} setErrors={setErrors} students={students} />}

            <Staffs errors={errors} setErrors={setErrors} instructors={instructors} />
          </form>
          ...
          <div className="ct-d-r-center-v w-100 m-3 ip-f-btn-group ct-btn-group">
            <Button.Group>
              ...
              />
              {!newCourse && (
                <Button uppercase color="teal transparent" text="Cancel" onClick={onClose} />
              )}
            </Button.Group>
          </div>
          ...
          {errors.length > 0 && (
            <div className="d-flex flex-row justify-content-end">
              <div className="ip-f-courses-error">Please fix the errors above before saving.</div>
            </div>
          )}
        </div>
      </div>
...
export const EditCourse = connectWithRedux(
  ...
  ['setOffering', 'setIsEditingOffering'],
);
