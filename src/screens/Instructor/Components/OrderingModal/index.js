  ...
  return (
    <Draggable draggableId={item.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <span className="ip-om-item-drag-icon">
            <i className="material-icons">drag_handle</i>
          </span>
          <span className="ip-om-item-icon">
            <i className="material-icons">{icon || 'video_library'}</i>
          </span>
...
function OrderingModalWithRedux({ ordering = ORD_INIT, setOrdering }) {
  ...
  const handleSave = () => {
    const result = _.map(lisitems, (item, index) => ({ ...item, index }));
  ...
  const onDragEnd = (result) => {
    ...
    const lisitems_ = reorder(lisitems, result.source.index, result.destination.index);
  ...
  return (
    <CTModal show middle closeOnBlur title={type} onSave={handleSave} onClose={handleClose}>
      ...
      <div className="ip-om" data-scroll>
        <div className="w-100 ct-d-r-center-v mb-4 ml-3">
          ...
          <Button outlined compact onClick={orderByDate} color={orderby === 'date' ? 'teal' : ''}>
            DATE
          </Button>
          <Button
            outlined
            ...
            {(provided) => (
              <div ref={provided.innerRef} {...provided.droppableProps}>
                {lisitems.map((item, index) => (
                  <OrdItem index={index} item={item} key={item.id} icon={icon} />
                ))}
