...
const GolssaryWithRedux = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await apiInstance.get(
        `/api/Glossary/ByCourseOffering?courseId=${selectCourse}&offeringId=${selectOffering}`,
        config,
      );
      setData(res.data);
    };
  ...
  return (
    <CTLayout {...layoutProps}>
      <h1>Glossary</h1>
      <br />
      <GlossaryBar setSelectCourse={setSelectCourse} setSelectOffering={setSelectOffering} />
      <br />
