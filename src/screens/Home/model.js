  ...
  effects: {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          yield put({
            ...
            payload: _.filter(
              data,
              (uni) => uni.id !== HomeConstants.UnknownUniversityID,
            ).reverse(),
          });
          return;
        }
    ...
    *getStarredOfferings({ payload }, { call, put, select }) {
      ...
      try {
        ...
        if (starredOfferings) {
          yield put({
            type: 'setStarredOfferings',
            payload: Object.keys(JSON.parse(starredOfferings)),
          });
        }
      } catch (error) {
        return [];
      ...
      try {
        ...
        yield put({
          ...
          payload: _.map(data, (depart) => {
            let departUni =
              university || _.find(homeState.universities, { id: depart.universityId });
    ...
    *initialize(__, { call, put, select, take }) {
      ...
      if (user.isLoggedIn) {
        // TODO: LOSE CONTROL OF USER
        const userUniId = user.getUserInfo().universityId;
        yield put({
          type: 'selectUniversity',
          payload: userUniId !== HomeConstants.UnknownUniversityID ? userUniId : null,
        });
      } else {
