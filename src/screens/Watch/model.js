...
function splitter(captionsArray) {
  ...
  for (let i = 0; i < captionsArray.length; i += 1) {
    ...
    if (toReturn.length === 0) {
      toReturn.push({
        startTime: captionsArray[0].startTime,
        endTime: captionsArray[0].endTime,
        text: captionsArray[0].text,
      });
      i += 1;
    }
  ...
  reducers: {
    ...
    setTranscript(state, { payload }) {
      ...
      for (let payloadIndex = 0; payloadIndex < payload.length; payloadIndex += 1) {
        ...
        if (
          payloadIndex !== 0 &&
          payload[payloadIndex - 1].startTime !== payload[payloadIndex].startTime
        ) {
        ...
        for (let i = 0; i < checkNumber && doInsert; i += 1) {
          ...
          doInsert =
            c.text !== caption.text || Math.abs(caption.startTime - c.startTime) > maxTimeDelta;
          if (state.updating) {
        ...
        for (
          let i = insertIndex;
          doInsert && i < result.length && i < insertIndex + checkNearby;
          i += 1
        ) {
          let c = result[i];
          doInsert =
        ...
        if (
          ...
          Math.abs(caption.startTime - caption.endTime) < 30
        ) {
      ...
      if (state.liveMode && state.transcript !== ARRAY_EMPTY) {
        if (!state.updating) {
          ...
          for (let i = 0; i < state.transcript.length; i += 1) {
            if (
              Number(state.transcript[i].startTime) <= j &&
              Number(state.transcript[i].endTime) >= j
            ) {
        ...
        if (payload !== undefined) {
          for (let i = 0; i < state.transcript.length; i += 1) {
            if (
              state.transcript[i].startTime <= payload.startTime + state.captionSpeedUp &&
              state.transcript[i].endTime >= payload.endTime + state.captionSpeedUp
            ) {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      ...
      history.listen((event) => {
        if (
          event.pathname === '/video' ||
          (event.action === 'PUSH' && event.location.pathname === '/video')
        ) {
