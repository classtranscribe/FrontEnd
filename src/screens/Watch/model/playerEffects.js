...
export const media_setCurrTime = createAsyncThunk('watch/media_setCurrTime',
  async ({ payload, realTime = false }, { dispatch, getState }) => {
    const { watch } = getState();
    if (watch.liveMode && !realTime) {
      dispatch(setLiveMode(payload > -5 ? 1 : 2));
      payload = watch.duration + payload;
    }
    if (PlayerData.video1) PlayerData.video1.currentTime = payload;
    if (PlayerData.video2) PlayerData.video2.currentTime = payload;
    if (PlayerData.aslVideo) PlayerData.aslVideo.currentTime = payload;
...
export const seekToPercentage = createAsyncThunk('watch/seekToPercentage',
  async (p = 0, { dispatch, getState }) => {
    ...
    dispatch(media_setCurrTime(seekTo, true));
  });
