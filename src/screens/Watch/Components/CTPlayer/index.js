...
import { PRIMARY, SECONDARY, PS_MODE, NESTED_MODE } from '../../Utils';
import './index.scss';
import './playerModes.css';

const videoRef1 = (node) => {
  PlayerData.video1 = node;
};
const videoRef2 = (node) => {
  PlayerData.video2 = node;
};
const ClassTranscribePlayerNew = (props) => {
  ...
  useEffect(() => {
    if (isTwoScreen && !isMobile) {
      dispatch({
        type: 'watch/setMode',
        payload: window.innerWidth <= 900 ? NESTED_MODE : PS_MODE,
      });
    }
  }, [isTwoScreen]);
  ...
  return (
    <>
      ...
      >
        {useHls ? <VideoHls {...media1Prop} /> : <Video {...media1Prop} />}
      </div>
      {isTwoScreen && (
        <div
...
export const ClassTranscribePlayer = connect(
  ({
    watch: { media, mode, isSwitched, isFullscreen, embedded },
    playerpref: { transView, muted, volume, playbackrate, openCC },
    loading,
  }) => ({
    media,
    mode,
