...
export function _generateDefaultEpubName(ePubs, defaultTitle) {
  ...
  if (ePubs.length > 0 && ePubs !== ARRAY_INIT) {
    // if there are previous epubs made
    let mediaName = defaultTitle; // the default title
    let mediaNameLen = mediaName.length;
    let maxAffix = -1;
    for (const epub of ePubs) {
      let title = epub.title;
      if (title.substring(0, mediaNameLen) === mediaName) {
        // if the title includes the default name
        let diff = title.length - mediaName.length;
        if (diff === 0) {
          // if the title is exactly the default name
