...
test.describe('Search Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('#sp-input')).toBeVisible({ timeout: 15000 });
  });
  ...
  test('search button is visible', async ({ page }) => {
    // SearchInput (pico-ui Button) renders with aria-label="search"
    // Use attribute selector since pico-ui may not render a standard <button> element
