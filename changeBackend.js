...
function change(data, url) {
  let serverEnv;
  if (url === backendList.production) serverEnv = 'window.env.REACT_APP_API_BASE_URL';
  if (url === backendList.dev) serverEnv = 'window.env.REACT_APP_TESTING_BASE_URL';
...
async function setCustomBackend({ localhost: url }) {
  if (!url.startsWith('http://localhost')) {
    logger.log(
      'error',
      chalk.red(
        `The server address must be complete \ni.e. ${chalk.bold('https://localhost[...]')}`,
      ),
    );
    return;
  }
...
function processArgs() {
  ...
  if (backend[0] === 'localhost') setCustomBackend(program.opts());
  else setBackend(backendList[backend[0]]);
}
